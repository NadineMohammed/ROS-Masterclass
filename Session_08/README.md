...
**Problem 1 — Map not displaying in RViz ("No map received")**
The Map display's Reliability Policy was initially set to `Best Effort`,
while `map_server` publishes `/map` with `Reliable` reliability and
`Transient Local` durability. Since these QoS settings did not match, the
subscription silently failed to receive any data, even though the topic
itself was publishing correctly (confirmed via `ros2 topic echo /map
--once`). Setting the Map display's Reliability Policy to `Reliable` (while
keeping Durability as `Transient Local`) resolved this immediately.

**Problem 2 — `map_saver_cli` failing with a "Magick: Unable to open file"
error**
This occurred because the destination `map/` folder did not exist yet before
running the save command. Creating the folder first with `mkdir -p` resolved
the issue.

**Problem 3 — AMCL not correcting itself after a wrong initial pose, even
while driving**
This was initially mistaken for a bug, but is actually expected AMCL
behavior: AMCL's particle filter searches locally around the given initial
pose hypothesis rather than performing a global search across the entire
map. If the true position is far from the clicked pose, no particles are
ever near the truth, so the filter cannot discover the correct position on
its own — providing an accurate initial pose via 2D Pose Estimate was
required for successful localization in every case tested.

**Problem 4 — ETGAH platform does not support custom world uploads**
Certain file extensions/folder structures required for a fully custom
Gazebo world could not be uploaded to the ETGAH platform. This was solved by
running the custom world (`obstacles.world`) locally in a native Ubuntu ROS 2
installation instead, completing the full mapping-then-localization workflow
outside of ETGAH and including all resulting proof (screenshots, node/topic
graphs, and a localization GIF) in the `custom_world/` folder of this
submission.

## 12. Submission Checklist

- [x] Used the map created in the previous SLAM assignment (`turtlebot3_world`)
- [x] Loaded the saved map using `map_server`
- [x] Launched AMCL without errors
- [x] Confirmed `/scan` and `/odom` are available
- [x] Displayed Map, TF, RobotModel, LaserScan, and ParticleCloud in RViz
- [x] Tested one wrong initial pose (main task + house bonus + custom world)
- [x] Tested one correct initial pose (main task + house bonus + custom world)
- [x] Drove the robot and confirmed localization remains stable
- [x] Confirmed AMCL publishes the `map → odom` transform
- [x] Confirmed `/amcl_pose` updates while the robot moves
- [x] Bonus: mapped and localized in a second world (`turtlebot3_house`)
- [x] Bonus: created and tested a custom world (`obstacles.world`) locally,
      due to an ETGAH platform upload limitation, with full proof included
